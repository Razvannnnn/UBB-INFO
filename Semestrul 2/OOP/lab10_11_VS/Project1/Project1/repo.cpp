    ...
    if (!exist(produs)) {
        ...
        for (Produs& p : produse) {
            if (p.getNume() == produs.getNume() && p.getTip() == produs.getTip() &&
                p.getProducator() == produs.getProducator() && p.getPret() == produs.getPret()) {
...
void testFILErepoWrite() {
    RepoProdusFile repo{ "C:\\Users\\razva\\Desktop\\OOP\\lab10_11_VS\\Project1\\Project1\\testFile.txt" };
    assert(repo.getAll().empty());
...
void testFILErepoRead() {
    RepoProdusFile repo{ "C:\\Users\\razva\\Desktop\\OOP\\lab10_11_VS\\Project1\\Project1\\test.txt" };
